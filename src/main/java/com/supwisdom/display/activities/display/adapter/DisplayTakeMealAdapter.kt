...
class DisplayTakeMealAdapter constructor(private val context: Context) : RecyclerView.Adapter<DisplayTakeMealAdapter.Holder>() {
    ...
    fun addItem(bean: DisplayMsgRecord) {
        if (checkRepeat(bean)) {
            return
        }
        list.add(0, bean)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(LayoutInflater.from(context).inflate(R.layout.item_display_takefood, parent, false))
    }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.vOrdernoInfo.text = bean.orderno
        holder.vOrderName.text = AppCommonUtil.getStuempnoSpec(bean.custname)
        holder.vTakeMealWindow.text = bean.windowname
    }

    inner class Holder constructor(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var vOrdernoInfo = itemView.findViewById(R.id.tv_orderno) as TextView
        var vOrderName = itemView.findViewById(R.id.tv_name) as TextView
        var vTakeMealWindow = itemView.findViewById(R.id.tv_takemeal_window) as TextView
    }
}